...
{{site.data.keyword.cloud}} {{site.data.keyword.ihsdbaas_mongodb_full}} サービスは、24 時間ごとに 1 回、データベース全体のバックアップを自動的にトリガーします。 これらの暗号化されたバックアップの直近 7 日分が、サポート対象地域のすべてのアベイラビリティー・ゾーンのローカル・ストレージ上に冗長的に保持されます。
{: shortdesc}
